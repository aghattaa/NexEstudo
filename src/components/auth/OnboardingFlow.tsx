...
import { Play, Sparkles, Brain, BookOpen, Target, ChevronRight, User, Lock, Mail, ArrowRight, UserPlus, LogIn } from 'lucide-react';
...
export default function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  // Helper to transition between steps with animation
  const nextStep = (targetStep: number, delayMs: number = 0) => {
    setAnimateOut(true);
    setTimeout(() => {
      setStep(targetStep);
      setAnimateOut(false);
  ...
  const LoginScreen = () => {
    ...
    const handleSubmit = () => {
      if (!isLogin && name.trim()) {
        ...
        updateUser({ name: 'Estudante Master', email });
      }
      nextStep(2);
    };
    ...
    return (
      <div className={`w-full h-screen bg-black flex relative overflow-hidden transition-opacity duration-700 ${animateOut ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        {/* Left Side - Vibrant Abstract Illustration */}
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 relative z-20">
          <div className="w-full max-w-[460px] bg-white/10 backdrop-blur-2xl p-10 rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-white/20 relative overflow-hidden group">
            ...
            <div className="flex bg-black/20 p-1.5 rounded-2xl border border-white/10 mb-10 relative z-20">
              <button 
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between pl-2">
                  ...
                  {isLogin && <button className="text-[10px] font-black text-cyan-400 hover:text-cyan-300 transition-colors tracking-wider">ESQUECI</button>}
                </div>
                <div className="relative group">
              ...
              <button 
                ...
                className={`w-full py-5 mt-4 text-white rounded-2xl font-black uppercase tracking-[0.2em] transition-all duration-300 hover:scale-[1.02] flex items-center justify-center gap-3 shadow-2xl ${isLogin ? 'bg-gradient-to-r from-cyan-500 to-blue-500 hover:shadow-[0_0_30px_rgba(6,182,212,0.6)]' : 'bg-gradient-to-r from-fuchsia-500 to-orange-500 hover:shadow-[0_0_30px_rgba(217,70,239,0.6)]'}`}
              >
                {isLogin ? 'Acessar Portal' : 'Explorar Agora'}
  ...
  const QuestionnaireScreen = () => {
    ...
    const handleContinue = () => {
      ...
      if (selectedMethod) {
        updateUser({ learningPreference: selectedMethod.name as any });
      }
      nextStep(4);
    };
    ...
    return (
      <div className={`w-full h-screen bg-gradient-to-br from-[#09090b] via-[#09090b] to-[#18181b] flex items-center justify-center relative overflow-hidden transition-opacity duration-500 ${animateOut ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="relative z-10 w-full max-w-5xl px-8 flex flex-col items-center">
          ...
          <p className="text-xl text-gray-400 text-center max-w-2xl mb-16 font-medium">Vamos adaptar sua experiência. Escolha o formato que mais funciona para você focar e absorver o conteúdo.</p>
            ...
            {methods.map((method) => {
              ...
              return (
                <button 
                  ...
                  className={`relative p-10 rounded-[32px] border-2 text-left transition-all duration-500 group overflow-hidden ${
                    isSelected 
                      ? 'border-white bg-white/10 shadow-[0_20px_50px_rgba(255,255,255,0.1)] scale-105' 
                      : 'border-white/5 bg-[#18181b] hover:border-white/20 hover:bg-[#27272a]'
                  }`}
                >
          ...
          <div className={`mt-16 transition-all duration-500 ${selected ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}>
             <button 
               ...
               className="px-12 py-5 bg-nexus-blue text-white rounded-2xl font-black uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(56,189,248,0.5)] hover:shadow-[0_0_50px_rgba(56,189,248,0.8)] hover:bg-nexus-blue-light flex items-center gap-4 transition-all duration-300 hover:scale-105"
             >
  ...
  switch (step) {
    case 0:
      return <LoadingScreen message="Iniciando Sistemas" />;
    case 1:
      return <LoginScreen />;
    case 2:
    ...
    case 4:
      return <LoadingScreen message="Personalizando Perfil" />;
    default:
      return null;
  }
}
